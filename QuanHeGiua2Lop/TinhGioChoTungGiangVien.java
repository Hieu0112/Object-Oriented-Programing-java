...
public class TinhGioChoTungGiangVien {
    public static void main(String[] args) {
        ...
        double sum=0;
        for (GiangVien gv:b) {
            if(ma.equals(gv.getMaGv())){
                System.out.println("Giang vien: "+gv.getNameGV());
                for (QuanLy re : res) {
                    for (MonHoc x:a) {
                        if(re.getMagv().equals(ma)  &&
                                re.getMamon().equals(x.getMa())){
                            System.out.println(x.getName()+" "+re.getSo());
                            sum+=re.getSo();
                        }
                    }
                }
